...
namespace thinkyoung {
    namespace wallet {
            ...
            void WalletImpl::upgrade_version() {
                ...
                try {
                    ...
                    if (current_version < 102) {
						self->set_transaction_fee(Asset(ALP_DEFAULT_TRANSACTION_FEE));
        ...
        void Wallet::set_data_directory(const path& data_dir) {
			...
			save_transaction_fee_configuration();
        }
        ...
        WalletTransactionEntry Wallet::register_contract(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing) {
            ...
            FC_ASSERT(asset_symbol == ALP_BLOCKCHAIN_SYMBOL, "asset_symbol must be TTCOIN");
            const auto asset_rec = data_ptr->get_asset_entry(asset_symbol);
            FC_ASSERT(asset_rec.valid(), "Asset not exist!");
            FC_ASSERT(asset_rec->id == 0, "asset_symbol must be TTCOIN");
            ...
            FC_ASSERT(register_fee.asset_id == 0, "register fee must be TTCOIN");
            FC_ASSERT(margin.asset_id == 0, "register fee must be TTCOIN");
            FC_ASSERT(fee.asset_id == 0, "register fee must be TTCOIN");
            trx.expiration = blockchain::now() + get_transaction_expiration();
            my->sign_transaction(trx, required_signatures);
        ...
        WalletTransactionEntry Wallet::transfer_asset_to_address(
            ...
            try {
				...
				//Add zxj 限制最低转账金额
				FC_ASSERT(amount_to_transfer >= ALP_DEFAULT_MIN_TRANSACTION_AMOUNT, "The transfer amount must be greater than 0.0001");
				...
				//mod zxj
				ShareType maxfee = get_max_transaction_fee();
				ShareType minfee = get_min_transaction_fee();
				double transfee = 1000 *get_transaction_fee_ratio() * amount_to_transfer/1000;
				if (transfee < minfee)
				{
					transfee = minfee;
				}
				...
				Asset required_fees((ShareType)transfee, 0);
				//
        ...
        thinkyoung::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract(
            ...
            try {
                ...
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
				//mod zxj 别的资产不能转
                //FC_ASSERT(amount_to_transfer_symbol == ALP_BLOCKCHAIN_SYMBOL, "Asset symbol should be TTCOIN");
                ...
                const auto asset_id = asset_rec->id;
				//mod zxj 别的资产不能转
                //FC_ASSERT(asset_id == 0, "Asset symbol should be TTCOIN");
                const int64_t precision = asset_rec->precision ? asset_rec->precision : 1;
				ShareType amount_to_transfer = static_cast<ShareType>(floor(real_amount_to_transfer * precision + 0.5));
                //Asset asset_to_transfer(amount_to_transfer, asset_id);
				//Add zxj 限制最低转账金额
				FC_ASSERT(amount_to_transfer >= ALP_DEFAULT_MIN_TRANSACTION_AMOUNT, "The transfer amount must be greater than 0.0001");
				...
				//mod zxj exec都按TTCOIN收费
				Asset asset_for_exec = to_asset(/*asset_rec->id*/0, ALP_BLOCKCHAIN_PRECISION, exec_cost);
                ...
                if (!is_testing) {
					...
					if (my->_blockchain->get_is_in_sandbox())
					{
						...
						{
							sandbox_get_enough_balances(from_account_name, asset_to_transfer, balances, required_signatures);
							sandbox_get_enough_balances(from_account_name, required_fees + asset_for_exec, balances, required_signatures);
						}						
					}
					else
					{
						...
						{
							//Asset asset_nll;
							get_enough_balances(from_account_name, asset_to_transfer, balances, required_signatures);
							get_enough_balances(from_account_name, required_fees + asset_for_exec, balances, required_signatures);
							//test mod 
							//required_signatures.insert(sender_account_address);
							//my->transfer_to_contract_trx(trx, to_contract_address, asset_to_transfer, asset_nll, asset_nll, sender_public_key, balances);
							
							//my->withdraw_to_transaction(required_fees + asset_for_exec,
							//	from_account_name,
							//	trx,
							//	required_signatures);
							//map<BalanceIdType, ShareType> balances1;
							//get_enough_balances(from_account_name, required_fees + asset_for_exec, balances1, required_signatures);
							//my->transfer_to_contract_trx(trx, to_contract_address, asset_nll, asset_for_exec, required_fees, sender_public_key, balances1);
						}
					}
                } else
                    required_signatures.insert(sender_account_address);
        ...
        WalletTransactionEntry Wallet::upgrade_contract(
            ...
            try {
                ...
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not TTCOIN!");
        ...
        WalletTransactionEntry Wallet::destroy_contract(
            ...
            try {
                ...
                if (balance_entry.valid()) {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not TTCOIN!");
                }
        ...
        WalletTransactionEntry Wallet::transfer_asset_to_many_address(
            ...
            try {
				...
				//Asset required_fees = get_transaction_fee();
				...
				//Add zxj 限制最低转账金额
				FC_ASSERT(total_asset_to_transfer.amount >= ALP_DEFAULT_MIN_TRANSACTION_AMOUNT, "The transfer amount must be greater than 0.0001");
				//mod zxj
				ShareType maxfee = get_max_transaction_fee();
				ShareType minfee = get_min_transaction_fee();
				double transfee = 1000*get_transaction_fee_ratio() * total_asset_to_transfer.amount/1000;
				if (transfee < minfee)
				{
					transfee = minfee;
				}
		...
		WalletTransactionEntry Wallet::update_transfer_config(
			const string& delegate_account_name,
			const string& min_transaction_fee,
			const string& max_transaction_fee,
			const string& transaction_fee_ratio)
		{
			try
			{
				FC_ASSERT(utilities::isNumber(min_transaction_fee), "inputed min_transaction_fee is not a number");
				FC_ASSERT(utilities::isNumber(max_transaction_fee), "inputed max_transaction_fee is not a number");
				FC_ASSERT(utilities::isNumber(transaction_fee_ratio), "inputed transaction_fee_ratio is not a number");
				SignedTransaction     trx;
				unordered_set<Address> required_signatures;
				trx.expiration = blockchain::now() + get_transaction_expiration();

				FC_ASSERT(wallet_get_delegate_statue(delegate_account_name), "update_transfer_config is only avaliable for delegates");
				auto from_account_address = get_owner_public_key(delegate_account_name);
				auto oname_rec = my->_blockchain->get_account_entry(delegate_account_name);

				const auto required_fees = get_transaction_fee();

				my->withdraw_to_transaction(required_fees,
					delegate_account_name,
					trx,
					required_signatures);
				required_signatures.insert(oname_rec->active_key());

				trx.update_transfer_config(delegate_account_name, min_transaction_fee,
					max_transaction_fee, transaction_fee_ratio);

				auto entry = LedgerEntry();
				entry.from_account = from_account_address;
				entry.to_account = from_account_address;
				entry.memo = "update transfer config";
				auto trans_entry = WalletTransactionEntry();
				trans_entry.ledger_entries.push_back(entry);
				trans_entry.fee = required_fees;
			...
			FC_CAPTURE_AND_RETHROW((delegate_account_name)(min_transaction_fee)(max_transaction_fee)(transaction_fee_ratio))
		}
        ...
        WalletTransactionEntry Wallet::create_asset(
            ...
            try {
				...
				auto required_fees = Asset(my->_blockchain->get_asset_registration_fee(symbol.size()), 0);//修改 
		...
		//void Wallet::set_transaction_fee_ratio(const double& ratio) {
		//	try {

		//		if (ratio < 0 || ratio > 1)
		//			FC_THROW_EXCEPTION(invalid_fee_ratio, "Invalid transaction fee ratio!", ("ratio", ratio));

		//		my->_blockchain->store_property_entry(PropertyIdType::transaction_fee_ratio, variant(ratio));
		//		//my->_wallet_db.set_property(transaction_fee_ratio, variant(ratio));
		//		save_transaction_fee_configuration();
		//	}
		//	catch (...) {
		//	}
		...
		//void Wallet::set_min_transaction_fee(const int& fee) {
		//	try {

		//		if (fee < ALP_DEFAULT_MIN_TRANSACTION_FEE )
		//			FC_THROW_EXCEPTION(invalid_fee, "Invalid transaction fee!", ("fee", fee));

		//		my->_blockchain->store_property_entry(PropertyIdType::transaction_min_fee, variant(fee));
		//		//my->_wallet_db.set_property(transaction_min_fee, variant(fee));
		//		save_transaction_fee_configuration();
		//	}
		//	catch (...) {
		//	}
		...
		//void Wallet::set_max_transaction_fee(const int& fee) {
		//	try {
		//		if (fee < ALP_DEFAULT_MIN_TRANSACTION_FEE )
		//			FC_THROW_EXCEPTION(invalid_fee, "Invalid transaction fee!", ("fee", fee));

		//		my->_blockchain->store_property_entry(PropertyIdType::transaction_max_fee, variant(fee));
		//		//my->_wallet_db.set_property(transaction_max_fee, variant(fee));
		//		save_transaction_fee_configuration();
		//	}
		//	catch (...) {
		//	}
		...
		void Wallet::save_transaction_fee_configuration( )
		{
			std::string dir = (get_data_directory()).string();
			int npos = dir.rfind("wallets");
			...
			if (fc::exists(db_path))
			{
				...
				TransactionFeeConfiguration _transfee_configuration;
				_transfee_configuration.min_transaction_fee = get_min_transaction_fee();
				_transfee_configuration.max_transaction_fee = get_max_transaction_fee();
				_transfee_configuration.transaction_fee_ratio = get_transaction_fee_ratio();

				try
				{
					fc::json::save_to_file(_transfee_configuration, configuration_file_name);
				}
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const {
            try {
				...
				Asset xts_fee(ALP_DEFAULT_TRANSACTION_FEE, 0);
		...
		double Wallet::get_transaction_fee_ratio( )const {
			{
				double fee_ratio = ALP_DEFAULT_TRANSACTION_FEE_RATIO;

				try {
					//double ratio = my->_wallet_db.get_property(transaction_fee_ratio).as<double>();
					const oPropertyEntry entry = my->_blockchain->get_property_entry(PropertyIdType::transaction_fee_ratio);
					FC_ASSERT(entry.valid(), "Getting transaction_fee_ratio property from property db failed!");
					double ratio = entry->value.as<double>();
					if (ratio > 0)
					{
						fee_ratio = ratio;
					}
		...
		int Wallet::get_min_transaction_fee()const { 
			{
				int xts_fee = ALP_DEFAULT_MIN_TRANSACTION_FEE;

				try {
					//int get_fee = my->_wallet_db.get_property(transaction_min_fee).as<int>();
					const oPropertyEntry entry = my->_blockchain->get_property_entry(PropertyIdType::transaction_min_fee);
					FC_ASSERT(entry.valid(), "Getting min_transaction_fee property from property db failed!");
					int get_fee = entry->value.as<int>();
					if (get_fee > 0)
					{
						xts_fee = get_fee;
					}
		...
		int Wallet::get_max_transaction_fee()const {
			{
				int xts_fee = ALP_DEFAULT_MAX_TRANSACTION_FEE;

				try {
					
					//int get_fee = my->_wallet_db.get_property(transaction_max_fee).as<int>();
					const oPropertyEntry entry = my->_blockchain->get_property_entry(PropertyIdType::transaction_max_fee);
					FC_ASSERT(entry.valid(), "Getting max_transaction_fee property from property db failed!");
					int get_fee = entry->value.as<int>();
					if (get_fee > 0)
					{
						xts_fee = get_fee;
