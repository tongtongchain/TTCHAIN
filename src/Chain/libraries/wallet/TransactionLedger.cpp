...
void WalletImpl::scan_balances()
{
    ...
    for (const auto& account : my_accounts)
    {
		...
		string tempentryID = (string)entry_id;
    ...
    const auto scan_balance = [&](const BalanceEntry& bal_rec)
    {
		...
		if (address == "TTC44neYLDPbZmbNd6S1tpeV5nBYKAsAcKKc"){
			int temp = 0;
		}
        if (!owner.valid())
            return;
    ...
    bool bNeedcreate)
{
    try {
		...
		auto isHaveWithdraw = false;
		auto isHaveDeposit = false;
		for (const auto& op : transaction.operations)
		{
		...
		if (isHaveWithdraw && isHaveDeposit)
		{
...
bool thinkyoung::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		...
		const oBalanceEntry bal_rec = _blockchain->get_balance_entry(op.balance_id);
		FC_ASSERT(bal_rec.valid());
		const auto amount = Asset(op.amount, bal_rec->asset_id());
...
bool thinkyoung::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		//mod zxj
		const auto amount = Asset(op.amount.amount,op.amount.asset_id); // Always base asset
...
bool thinkyoung::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
		...
		//mod zxj
		const auto opamount = Asset(op.amount.amount, op.amount.asset_id); // Always base asset
		if (opamount.asset_id == total_fee.asset_id)
			total_fee -= opamount;
...
bool thinkyoung::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		...
		for (auto& it : trx_rec.ledger_entries)
		{
			...
			{
				it.amount = Asset(op.amount,op.condition.asset_id);
				found = true;
				break;
			}
		}
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        const auto& transactions = my->_wallet_db.get_transactions();
        AssetIdType asset_id = 0;
        if (!asset_symbol.empty() && asset_symbol != ALP_BLOCKCHAIN_SYMBOL)//查单一类型
        {
            try
            {
                asset_id = my->_blockchain->get_asset_id(asset_symbol);
        ...
        for (const auto& item : transactions)
        {
			const auto& tempTx_entry = TransactionData(item.second);//过滤与查询 类型不匹配的资产
			if (tempTx_entry.ledger_entries.empty()) continue; /* TODO: Temporary */
			//PrettyTransaction pretty_trx = to_pretty_trx(tempTx_entry);
			/*if (!asset_symbol.empty()){
				vector<LedgerEntry> temp;
				for (const auto& entry : tempTx_entry.ledger_entries){
					if (entry.amount.amount > 0 && entry.amount.asset_id == asset_id){
						temp.push_back(entry);
					}
				}
				tempTx_entry.ledger_entries = temp;
			}*/

			const auto& tx_entry = tempTx_entry;
            if (tx_entry.ledger_entries.empty()) continue; /* TODO: Temporary */
            ...
            if (asset_id != 0/*!asset_symbol.empty()*/)//查询资产是否匹配
            {
                bool match = false;
                for (const auto& entry : tx_entry.ledger_entries)
                    match |= entry.amount.amount > 0 && entry.amount.asset_id == asset_id;
    ...
    bool wither_splite)const
{
    try {
		...
		for (const auto& item : history){//添加
			PrettyTransaction pretty_trx = to_pretty_trx(item);
			if (pretty_trx.trx_type == thinkyoung::blockchain::TransactionType::create_asset && trx_splite == trx_type_desipate){
				continue;
			}//如果是创建资产并且是充值option，跳过
        ...
        for (const auto& name : account_names)
        {
			...
			vector<PrettyTransaction>::iterator trx = pretties.begin();//添加
			bool is_abolished = false;
            for (auto& trx : pretties)
            {
				is_abolished = trx.is_abolished;
                    ...
                    if (from_me)
                    {
                       ...
                       /* if ((running_balances[amount_asset_id] - entry.amount) >= Asset(0, amount_asset_id))
						...
						else if (trx.trx_type == thinkyoung::blockchain::TransactionType::transfer_multi_asset)
						{
							...
							/*else if (entry->amount.amount == trx.fee.amount && entry->amount.asset_id == trx.fee.asset_id)
							{
								entry->amount = Asset(0, 0);
							}*/
  ...
  /*                      if (!trx.is_virtual && !any_from_me)
                    ...
                    to_me |= (entry->to_account.find(name + " ") == 0);
					//mod zxj 
					if (entry->from_account.empty() && to_me)
					{
						entry = trx.ledger_entries.erase(entry);
						continue;
					}

					if (entry->from_account.empty() && !to_me && Address::is_valid(entry->to_account, ALP_ADDRESS_PREFIX)){
						Address to_address = Address(entry->to_account);
						const auto account_entry = get_account_for_address(to_address);
						if (account_entry.valid()){
							to_me |= (account_entry->name == name);
						}
					}
					/* If payer != sender */
					/*if (!to_me && Address::is_valid(entry->to_account,ALP_ADDRESS_PREFIX)){
						Address to_address = Address(entry->to_account);
						const auto account_entry = get_account_for_address(to_address);
						const auto account_entry_name = get_account(account_name);
						if (account_entry.valid()){
							to_me |= (account_entry->name == account_name || (account_entry_name.active_key() == account_entry->active_key()) || (account_entry_name.active_address() == account_entry->active_address()));
							to_me |= account_entry_name.owner_address() == account_entry->owner_address();
						}
					}*/
					
					//添加 && entry.amount.amount != 0
					if (to_me && entry->amount.amount != 0){
						if (!is_abolished)
							running_balances[amount_asset_id] += entry->amount;
					}
		...
		/*vector<PrettyTransaction> tempPretties;
		tempPretties.reserve(pretties.size());
		for (auto& entrys : pretties)
		{
			bool  flag = false;
			for (auto& temp : entrys.ledger_entries){
				if (temp.amount.asset_id == asset_id){
					tempPretties.push_back(entrys);
				}
			}
		}*/
		for (auto& entrys : pretties)
        {
            ...
            for (; iter != entrys.ledger_entries.end();)
            {
				...
				if (iter->amount == Asset(0, iter->amount.asset_id) && entrys.trx_type == thinkyoung::blockchain::TransactionType::transfer_multi_asset){
					memoTemp = "multi-asset transfer's fee ";
				}
				if (iter->amount == Asset(0, iter->amount.asset_id) && entrys.trx_type == thinkyoung::blockchain::TransactionType::register_account_transaction){
					memoTemp = "register " + iter->to_account;
				}
...
PrettyTransaction Wallet::to_pretty_trx(const WalletTransactionEntry& trx_rec) const
{
	...
	//add zxj
	if ((pretty_trx.is_confirmed) && !(my->_blockchain->get_transaction(pretty_trx.trx_id, false).valid()))
	{
		if (!pretty_trx.is_virtual)
		{
			pretty_trx.is_abolished = true;
		}
    ...
    for (const auto& entry : trx_rec.ledger_entries)
    {
        ...
        else
        {
            ...
            for (const auto & op : trx_rec.trx.operations)
            {
				...
				case register_account_op_type:
				{
					...
					//pretty_entry.to_account = (string)(*(register_op.condition.owner()));
        ...
        for (const auto & op : trx_rec.trx.operations)
        {
			...
			{
				pretty_trx.is_abolished = false;
				pretty_trx.trx_id = trx_rec.entry_id;
				if (!(my->_blockchain->get_transaction(pretty_trx.trx_id, false).valid()))
				{
					pretty_trx.is_abolished = true;
				}
				break;
