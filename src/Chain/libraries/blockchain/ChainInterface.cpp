    ...
    namespace blockchain {
        ...
        bool ChainInterface::is_valid_symbol_name(const string& symbol)const
        {
            try {
                ...
                FC_ASSERT(symbol != "TTCOIN", "Symbol must not be TTCOIN");
                ...
                if (symbol.find("TTCOIN") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with TTCOIN");
        ...
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
			...
			//FC_ASSERT(symbol_length >= 3, "symbol_length must bigger tran 3");
			//FC_ASSERT(symbol_length <= 16, "symbol_length must not bigger tran 16");
			////ShareType maxFee = 6200000 * ALP_BLOCKCHAIN_PRECISION;
			//int len = symbol_length - 3;
			///*if (len == 0){
			//	return 5000000 * ALP_BLOCKCHAIN_PRECISION;
			//}*/
			//return maxFee -= len * 200000 * ALP_BLOCKCHAIN_PRECISION;
			...
			static const ShareType symbol_price[] = { 620000000000,600000000000, 580000000000, 560000000000, 540000000000, 520000000000, 500000000000, 480000000000, 460000000000, 440000000000, 420000000000,
				400000000000, 380000000000, 360000000000 };
		...
		//mod zxj
		BalanceIdType ChainInterface::get_balanceid(const Address& contract_address, WithdrawBalanceTypes balance_type, AssetIdType asset_id_arg)
        {
            WithdrawWithSignature withdraw = WithdrawWithSignature(contract_address);
			WithdrawCondition cond = WithdrawCondition(withdraw, asset_id_arg, 0, balance_type);
            ...
            SignedTransaction& trx)
        {
			...
			//mod zxj
			BalanceIdType balance_id = get_balanceid(from_contract_address, WithdrawBalanceTypes::withdraw_contract_type, amount_to_withdraw.asset_id);
