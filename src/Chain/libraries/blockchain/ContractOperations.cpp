    ...
    namespace blockchain
    {
        ...
        ShareType check_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_check, const Address& address_check)
        {
            ...
            while (it != balances_check.end())
            {
				...
				//mod zxj
                //FC_ASSERT(current_balance_entry->asset_id() == 0, "Asset must use TTCOIN");
                ...
                if (!issuer_override)
                {
                    ...
                    case withdraw_signature_type:
                    {
						...
						//mod zxj
						if (current_balance_entry->asset_id() == 0)
						{
		...
		vector<ShareType> check_muti_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_check, const Address& address_check)
		{
			std::map<BalanceIdType, ShareType>::const_iterator it = balances_check.begin();
			vector<ShareType> all_amount(10);
			ShareType tmp_amount = 0;
			ShareType asset_id = 0;
			...
			{ 
				...
				asset_id = current_balance_entry->asset_id();
				...
				all_amount[asset_id] = get_amount_sum(all_amount[asset_id], it->second);
		...
		void withdraw_enough_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_from, ShareType amount_required, std::map<BalanceIdType, ShareType>& balances_to, ShareType assetid = 0)
        {
            balances_to.clear();
            std::map<BalanceIdType, ShareType>::const_iterator it = balances_from.begin();
			ShareType balance_assetid;
			oBalanceEntry current_balance_entry;
            while (it != balances_from.end())
            {
				current_balance_entry = eval_state._current_state->get_balance_entry(it->first);
				balance_assetid = current_balance_entry->asset_id();
				if (balance_assetid != assetid)
					continue;
		...
		void withdraw_enough_muti_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_from, Asset transaction_amount, Asset transaction_fee, std::map<BalanceIdType, ShareType>& balances_to)
		{
			balances_to.clear();
			std::map<BalanceIdType, ShareType>::const_iterator it = balances_from.begin();
			ShareType fee_assetid = transaction_fee.asset_id;
			ShareType amount_assetid = transaction_amount.asset_id;
			ShareType fee_amount = transaction_fee.amount;
			ShareType amount = transaction_amount.amount;
			ShareType balance_assetid;
			oBalanceEntry current_balance_entry;
			bool findfee = false;
			...
			while (it != balances_from.end())
			{
				...
				if (balance_assetid == fee_assetid && !findfee)
				{
					if (fee_amount <= it->second)
					{
						balances_to.insert(std::pair<BalanceIdType, ShareType>(it->first, fee_amount));
						findfee = true;
						if (findamount)
						{
							break;
					...
					{
						balances_to.insert(std::pair<BalanceIdType, ShareType>(it->first, it->second));
						fee_amount = fee_amount - it->second;
					}
					++it;
				}
				else if (balance_assetid == amount_assetid && !findamount)
				{
					if (amount <= it->second)
					{
						balances_to.insert(std::pair<BalanceIdType, ShareType>(it->first, amount));
						findamount = true;
						if (findfee)
						{
							break;
			...
			if (it == balances_from.end())
				FC_CAPTURE_AND_THROW(insufficient_funds, (amount));
		}
            ...
            try
            {
                ...
                if (!has_on_upgrade)
                {
                    ...
                    if (!eval_state.evaluate_contract_testing)
                    {
						...
						withdraw_enough_balances(eval_state, balances, required, withdraw_map);
                    ...
                    if (!eval_state.skipexec)
                    {
                        ...
                        catch (contract_error&e)
                        {
                            if (!eval_state.evaluate_contract_testing)
                            {
								...
								withdraw_enough_balances(eval_state, balances, (exec_cost + transaction_fee).amount, withdraw_map);
        ...
        void DestroyContractOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try
            {
                ...
                */
				...
				//mod zxj
				int nAsset = eval_state._current_state->last_asset_id() + 1;
				BalanceIdType contract_balance_id;
				vector <oBalanceEntry> vet_contract_balance_entry(20);
				for (int i = 0; i < nAsset; i++)
				{
					contract_balance_id = eval_state._current_state->get_balanceid(entry->id, WithdrawBalanceTypes::withdraw_contract_type,i);
					vet_contract_balance_entry[i] = (eval_state._current_state->get_balance_entry(contract_balance_id));
				}
                ...
                if (!has_on_destroy)
                {
                    ...
                    if (!eval_state.evaluate_contract_testing)
                    {
						...
						for (int i = 0; i < nAsset; i++)
						{
							if (vet_contract_balance_entry[i].valid())
								eval_state.p_result_trx.operations.push_back(OnDestroyOperation(id, Asset(vet_contract_balance_entry[i]->balance, i)));
							else
                ...
                else  // has on destroy function
                {
                    ...
                    if (!eval_state.skipexec)
                    {
                        ...
                        try
                        {
							...
							for (int i = 0; i < nAsset; i++)
							{
								eval_state._contract_balance_remain[i] = 0;
								if (vet_contract_balance_entry[i].valid())
									eval_state._contract_balance_remain[i] = vet_contract_balance_entry[i]->get_spendable_balance(eval_state._current_state->now()).amount;
							}
                            ...
                            if (!eval_state.evaluate_contract_testing)
                            {
								...
								for (int i = 0; i < nAsset; i++)
								{
									if (eval_state._contract_balance_remain[i] > 0)
										eval_state.p_result_trx.operations.push_back(OnDestroyOperation(id, Asset(eval_state._contract_balance_remain[i], i)));
								}
                                ShareType deposit_amount = margin_balance_entry->balance;
                                eval_state.p_result_trx.operations.push_back(WithdrawContractOperation(margin_balance_entry->id(), margin_balance_entry->balance));
                                eval_state.p_result_trx.operations.push_back(DepositOperation(Address(entry->owner), Asset(deposit_amount, 0)));
        ...
        void OnDestroyOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try
            {
				...
				//mod zxj
				BalanceIdType contract_balance_id = eval_state._current_state->get_balanceid(entry->id, WithdrawBalanceTypes::withdraw_contract_type, amount.asset_id);
                oBalanceEntry contract_balance_entry = eval_state._current_state->get_balance_entry(contract_balance_id);
                ...
                if (contract_balance_entry.valid())
                {
					...
					//mod zxj
					eval_state.add_balance(Asset(deposit_amount, amount.asset_id));
					...
					//mod zxj
					WithdrawCondition cond = WithdrawCondition(withdraw, amount.asset_id, 0, WithdrawBalanceTypes::withdraw_common_type);
            ...
            if (!eval_state.skipexec)
            {
                ...
                catch (const contract_error& e)
                {
                    if (!eval_state.evaluate_contract_testing)
                    {
						...
						withdraw_enough_balances(eval_state, balances, (exec_cost + eval_state.required_fees).amount, withdraw_map);
                        eval_state.p_result_trx.operations.resize(1);
                        eval_state.p_result_trx.expiration = eval_state.trx.expiration;
                        eval_state.p_result_trx.operations.push_back(BalancesWithdrawOperation(withdraw_map));
                    }
        ...
        void TransferContractOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
				...
				//mod zxj 别的资产不能转
                //FC_ASSERT(transfer_amount.asset_id == 0, "transfer asset must be TTCOIN");
                //FC_ASSERT(costlimit.asset_id == 0, "Execution of contract must cost TTCOIN");
                ...
                {
					//mod zxj 多资产不简单增加
					if (transfer_amount.asset_id != 0)
					{
						vector<ShareType> all_amount = check_muti_balances(eval_state, balances, Address(this->from));

						ShareType require = 0;
						ShareType require1 = 0;
						require = get_amount_sum(costlimit.amount, transaction_fee.amount);
						require1 = transfer_amount.amount;


						if (all_amount[0] < require && all_amount[transfer_amount.asset_id] < require1)
                ...
                if (!has_on_deposit)
                {
                    if (!eval_state.evaluate_contract_testing)
                    {
						...
						if (transfer_amount.asset_id != 0)
						{
							withdraw_enough_muti_balances(eval_state, balances, transfer_amount, transaction_fee, withdraw_map);
						}
						else
						{
							required = get_amount_sum(transaction_fee.amount, transfer_amount.amount);
							withdraw_enough_balances(eval_state, balances, required, withdraw_map);
						}
                    ...
                    if (!eval_state.skipexec)
                    {
                        ...
                        try
                        {
                            ...
                            */
							...
							if (transfer_amount.asset_id == 0)
							{
								required = get_amount_sum(exec_cost, transfer_amount.amount);
								required = get_amount_sum(required, transaction_fee.amount);
							}
