        ...
        namespace detail {
            ...
            */
            ...
            bool detail::ClientImpl::wallet_check_address(const std::string& address) {
                ...
                size_t first = address.find_first_of("TTC");
            ...
            WalletTransactionEntry ClientImpl::wallet_scan_transaction(const string& transaction_id, bool overwrite_existing) {
                ...
                try {
                    if (transaction_id == "ALL") {//表示全部transaction
			...
			WalletTransactionEntry ClientImpl::wallet_update_transfer_config(const std::string& delegate_account_name, const string& min_transaction_fee,
				...
				try {
					auto entry = _wallet->update_transfer_config(delegate_account_name,min_transaction_fee, max_transaction_fee, transaction_fee_ratio);
					_wallet->cache_transaction(entry);
					network_broadcast_transaction(entry.trx);
					return entry;
				}

				FC_RETHROW_EXCEPTIONS(warn, "", ("delegate_account_name", delegate_account_name)("min_transaction_fee", min_transaction_fee)("max_transaction_fee", max_transaction_fee)("transaction_fee_ratio", transaction_fee_ratio))
			}
