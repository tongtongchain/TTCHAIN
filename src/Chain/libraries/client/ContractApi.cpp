    ...
    namespace client {
        namespace detail {
            ...
            std::vector<thinkyoung::blockchain::BalanceEntry> ClientImpl::get_contract_balance(const std::string& contract) {
                ...
                contract_address = get_contract_address(contract);
				//mod zxj
				int nNum  = _chain_db->get_assets("",20).size();
				BalanceIdType contract_balance_id;
				oBalanceEntry entry;
				for (int i = 0; i < nNum; i++)
				{
					contract_balance_id = _chain_db->get_balanceid(contract_address, WithdrawBalanceTypes::withdraw_contract_type,i);
					entry = _chain_db->get_balance_entry(contract_balance_id);
