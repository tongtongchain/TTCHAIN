...
namespace thinkyoung {
    ...
    if( glog->obscure_passwords() )
      args.push_back( fc::variant("*********") );
		...
		thinkyoung::wallet::WalletTransactionEntry CommonApiClient::wallet_update_transfer_config(const std::string& delegate_account_name, const std::string& min_transaction_fee, const std::string& max_transaction_fee, const std::string& transaction_fee_ratio)
		{
			ilog("received RPC call: wallet_update_transfer_config(${min_transaction_fee}, ${max_transaction_fee}, ${transaction_fee_ratio})", ("min_transaction_fee", min_transaction_fee)("max_transaction_fee", max_transaction_fee)("transaction_fee_ratio", transaction_fee_ratio));
			thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
			uint64_t call_id = 0;
			fc::variants args;
			if (glog != NULL)
			{
				args.push_back(fc::variant(delegate_account_name));
				args.push_back(fc::variant(min_transaction_fee));
				args.push_back(fc::variant(max_transaction_fee));
				args.push_back(fc::variant(transaction_fee_ratio));
				call_id = glog->log_call_started(this, "wallet_update_transfer_config", args);
			}
			...
			struct scope_exit
			{
				fc::time_point start_time;
				scope_exit() : start_time(fc::time_point::now()) {}
				~scope_exit() { dlog("RPC call wallet_update_transfer_config finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
			} execution_time_logger;
			try
			{
				thinkyoung::wallet::WalletTransactionEntry result = get_impl()->wallet_update_transfer_config(delegate_account_name,min_transaction_fee, max_transaction_fee, transaction_fee_ratio);
				if (call_id != 0)
					glog->log_call_finished(call_id, this, "wallet_update_transfer_config", args, fc::variant(result));
