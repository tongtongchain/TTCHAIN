...
namespace thinkyoung {
    namespace rpc_stubs {
		...
		//add zxj
		thinkyoung::wallet::WalletTransactionEntry CommonApiRpcClient::wallet_update_transfer_config(const std::string& delegate_account_name, const std::string& min_transaction_fee, const std::string& max_transaction_fee, const std::string& transaction_fee_ratio)
		{
			fc::variant result = get_json_connection()->async_call("wallet_update_transfer_config", std::vector < fc::variant > {fc::variant(delegate_account_name), fc::variant(min_transaction_fee), fc::variant(max_transaction_fee), fc::variant(transaction_fee_ratio)}).wait();
			return result.as<thinkyoung::wallet::WalletTransactionEntry>();
		}
