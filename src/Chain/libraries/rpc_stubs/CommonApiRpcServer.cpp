...
namespace thinkyoung {
    namespace rpc_stubs {
        ...
        fc::variant CommonApiRpcServer::blockchain_list_active_delegates_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
        {
            ...
            (fc::json::from_string("11").as<uint32_t>()) :
            parameters[1].as<uint32_t>();
		...
		//Add zxj
		fc::variant CommonApiRpcServer::wallet_update_transfer_config_positional(fc::rpc::json_connection* json_connection, const fc::variants& parameters)
		{
			// check all of this method's prerequisites
			verify_json_connection_is_authenticated(json_connection);
			// done checking prerequisites
			...
			if (parameters.size() <= 0)
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 1 (delegate_account_name)");
			std::string delegate_account_name = parameters[0].as<std::string>();
			if (parameters.size() <= 1)
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 2 (min_transaction_fee)");
			std::string min_transaction_fee = parameters[1].as<std::string>();
			if (parameters.size() <= 2)
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 3 (max_transaction_fee)");
			std::string max_transaction_fee = parameters[2].as<std::string>();
			if (parameters.size() <= 3)
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 4 (transaction_fee_ratio)");
			std::string transaction_fee_ratio = parameters[3].as<std::string>();

			thinkyoung::wallet::WalletTransactionEntry result = get_client()->wallet_update_transfer_config(delegate_account_name, min_transaction_fee, max_transaction_fee, transaction_fee_ratio);
			return fc::variant(result);
		}

		fc::variant CommonApiRpcServer::wallet_update_transfer_config_named(fc::rpc::json_connection* json_connection, const fc::variant_object& parameters)
		{
			// check all of this method's prerequisites
			verify_json_connection_is_authenticated(json_connection);
			// done checking prerequisites

			if (!parameters.contains("delegate_account_name"))
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 'delegate_account_name'");
			std::string delegate_account_name = parameters["delegate_account_name"].as<std::string>();
			if (!parameters.contains("min_transaction_fee"))
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 'min_transaction_fee'");
			std::string min_transaction_fee = parameters["min_transaction_fee"].as<std::string>();
			if (!parameters.contains("max_transaction_fee"))
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 'max_transaction_fee'");
			std::string max_transaction_fee = parameters["max_transaction_fee"].as<std::string>();
			if (!parameters.contains("transaction_fee_ratio"))
				FC_THROW_EXCEPTION(fc::invalid_arg_exception, "missing required parameter 'transaction_fee_ratio'");
			std::string transaction_fee_ratio = parameters["transaction_fee_ratio"].as<std::string>();
        ...
        void CommonApiRpcServer::register_common_api_methods(const fc::rpc::json_connection_ptr& json_connection)
        {
		   ...
		   //add zxj
			bound_positional_method = boost::bind(&CommonApiRpcServer::wallet_update_transfer_config_positional,
				this, capture_con, _1);
			json_connection->add_method("wallet_update_transfer_config", bound_positional_method);
			bound_named_method = boost::bind(&CommonApiRpcServer::wallet_update_transfer_config_named,
				this, capture_con, _1);
			json_connection->add_named_param_method("wallet_update_transfer_config", bound_named_method);
        ...
        void CommonApiRpcServer::register_common_api_method_metadata()
        {
			...
			//add zxj
			{
				// register method wallet_update_transfer_config
				thinkyoung::api::MethodData wallet_update_transfer_config_method_metadata{ "wallet_update_transfer_config", nullptr,
					/* description */ "Updates the transfer fee config",
					/* returns */ "transaction_entry",
					/* params: */{
							{ "delegate_account_name", "account_name", thinkyoung::api::required_positional, fc::ovariant() },
							{ "min_transaction_fee", "uint8_t", thinkyoung::api::required_positional, fc::ovariant() },
							{ "max_transaction_fee", "uint8_t", thinkyoung::api::required_positional, fc::ovariant() },
							{ "transaction_fee_ratio", "double", thinkyoung::api::required_positional, fc::ovariant() }
					},
					/* prerequisites */ (thinkyoung::api::MethodPrerequisites) 4,
					/* detailed description */ "Updates the transfer fee config\n\nParameters:\n  delegate_account_name(delegate_account_name, required) : the delegate account that update transfer config\n  min_transaction_fee(int, required) : the min transaction fee number\n   max_transaction_fee(int, required) : the max transaction fee number\n  transaction_fee_ratio(double, required) : the transaction fee in accordance with the ratio of charge\n\nReturns:\n  transaction_entry\n",
					/* aliases */{}, false };
				store_method_metadata(wallet_update_transfer_config_method_metadata);
			}
        ...
        fc::variant CommonApiRpcServer::direct_invoke_positional_method(const std::string& method_name, const fc::variants& parameters)
        {
			...
			if (method_name == "wallet_update_transfer_config")
				return wallet_update_transfer_config_positional(nullptr, parameters);
