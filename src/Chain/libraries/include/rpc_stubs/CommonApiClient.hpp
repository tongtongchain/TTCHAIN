...
namespace thinkyoung {
    namespace rpc_stubs {
        ...
        public:
            ...
            std::vector<thinkyoung::blockchain::AccountEntry> blockchain_list_active_delegates(uint32_t first = fc::json::from_string("0").as<uint32_t>(), uint32_t count = fc::json::from_string("11").as<uint32_t>()) const override;
			...
			//add zxj 更新手续费策略
			thinkyoung::wallet::WalletTransactionEntry wallet_update_transfer_config(const std::string& delegate_account_name, const std::string& min_transaction_fee, const std::string& max_transaction_fee, const std::string& transaction_fee_ratio) override;
